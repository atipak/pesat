...
void deepNavigation::run()
{
  ...
  while (ros::ok())
  {
    ...
    ROS_DEBUG("Desired_Forward_Velocity [0-1]: %.3f ", desired_forward_velocity_);
    ...
    ROS_DEBUG("Desired_Angular_Velocity[0-1]: %.3f ", desired_angular_velocity_);
    ...
    if (use_network_out_)
    {
        ...
        ROS_INFO_ONCE("NOT PUBLISHING VELOCITY");

    ROS_DEBUG("Collision Prob.: %.3f - OutSteer: %.3f", probability_of_collision_, steering_angle_);
    ROS_DEBUG("--------------------------------------------------");
