...
class Math():
    ...
    @staticmethod
    def normalize_generalized(v):
        norm = np.linalg.norm(v, axis=(len(v.shape) - 1))
        norm[norm < 0.0001] = 1
        return v / norm.reshape(norm.shape + (1,))
    ...
    @staticmethod
    def uniform_circle_vectorized(length, size):
        size = (size,) + length.shape
        length = np.sqrt(np.random.uniform(np.zeros(length.shape), length, size))
        angle = np.pi * np.random.uniform(np.zeros(length.shape), 2, size)
        x = length * np.cos(angle)
        y = length * np.sin(angle)
        zeros_shape = size[::-1] + (2,)
        array = np.zeros(zeros_shape)
        array[..., 0] = x.T
        array[..., 1] = y.T
        return array
...
class Map():
    ...
    def map_from_file(cls, file_name, resolution=2):
        if os.path.isfile(file_name):
            ...
            try:
                ...
                map = Map(m, m, max_altitude=information["world"]["maximal_height"], resolution=resolution)
        ...
        obstacles = [ar, ar]
        map = Map(obstacles, obstacles, max_altitude=20, resolution=resolution)
        return map
    ...
    def find_nearest_free_position(self, x, y):
        ...
        nearest_in_cut_map = indices[
            np.argmin(np.sum(np.abs(indices - np.array([int(cut_map.shape[0]/2), int(cut_map.shape[1]/2)])), 1))]
